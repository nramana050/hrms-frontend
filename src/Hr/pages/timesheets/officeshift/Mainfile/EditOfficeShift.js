...
const Editshift = () => {
  ...
  const loadshift = async () => {
    const result = await axios.get(
      `http://13.126.190.50:5000/officeshifts/get/${id}`
    );
  ...
  const updateshift = async (e) => {
    e.preventDefault();
    await axios.put(`http://13.126.190.50:5000/officeshifts/update/${id}`, shift);
            ...
            <form onSubmit={(e) => updateshift(e)}>
              ...
              <div className="data-buttons">
                ...
                <Button
                  id="input-btn-cancel"
                  variant="outlined"
                  onClick={() => navigate("/hr/timesheets/officeshifts")}
                >
                  Back
                </Button>
              </div>
