...
export const saveOfficeShift = async (formData) => {
  try {
    await axios.post(
      "http://13.126.190.50:5000/officeshifts/create/officeShifts",
      formData
    );
  } catch (error) {
...
export const deleteOfficeShift = async (id) => {
  try {
    await axios.delete(`http://13.126.190.50:5000/officeshifts/delete/${id}`);
  } catch (error) {
...
export const loadOfficeShift = async () => {
  try {
    const result = await axios.get(
      "http://13.126.190.50:5000/officeshifts/get/officeShifts",
      {
        validateStatus: () => {
          return true;
        },
