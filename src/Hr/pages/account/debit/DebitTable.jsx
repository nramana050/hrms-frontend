...
`;
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - debit.length) : 0;
  ...
  const convertToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(debit);
  ...
  return (
    ...
    <tbody className="text-center">
      { debit.length === 0 ? renderDebitData() :(rowsPerPage > 0
            ? debit.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : debit
          ).filter((elem)=>{
            if(search.length===0)
              return elem;
