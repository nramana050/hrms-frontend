...
const AccountBalanceTable = ({ accountBalance, setRecDelete }) => {
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - accountBalance.length)
      : 0;
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: accountBalance.map((row) => [
          row.accountbalanceId,
          row.employeeName,
          row.employeeId,
          row.hsaBalance,
          row.fsaBalance,
  ...
  const convertToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(accountBalance);
  ...
  const renderAccountBalanceData = () => {
    return (
      <tr>
        ...
        <table className="table table-bordered table-hover shadow">
          ...
          <tbody className="text-center">
            {accountBalance?.length === 0
              ? renderAccountBalanceData()
              : (rowsPerPage > 0
                  ? accountBalance?.slice(
                      page * rowsPerPage,
                  ...
                  .map((accountBalance, index) => (
                    <tr key={accountBalance.accountBalanceId}>
                      ...
                      <td>{accountBalance.accountBalanceId}</td>
                      ...
                      <td className="mx-2">
                        <button
                          ...
                          onClick={() =>
                            handleDelete(accountBalance.accountBalanceId)
                          }
                        >
                          <FaTrashAlt />
                        </button>
