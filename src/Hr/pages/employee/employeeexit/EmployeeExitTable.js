...
const EmployeeExitTable = ({ employeeExit, setRecDelete,setOpen }) => {
...
`;
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - employeeExit.length) : 0;
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: employeeExit.map((row) => [
          row.employeeExitId,
          row.employeeToExit,
          row.exitDate,
          row.typeOfExit,
          row.exitInterview,
          row.inactivateEmployeeAccount,
          row.description,
  ...
  const convertToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(employeeExit);
    ...
    XLSX.writeFile(wb, "employeeexit.xlsx");
  };
  ...
  >
    <div className=" table-ka-top-btns" style={{marginTop:'-10px'}}>
    ...
    <div className="d-flex mt-4 four-btn" style={{ gap: "10px" }} y>
      ...
      <table id='table' className="table table-bordered table-hover shadow">
        ...
        <tbody className="text-center">
          { employeeExit.length === 0 ? renderEmployeeExitData() : (rowsPerPage > 0
            ? employeeExit.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : employeeExit
          ).filter((elem)=>{
            if(search.length===0)
              return elem;
              ...
              elem.inactivateEmployeeAccount.toLowerCase().includes(search.toLocaleLowerCase()) ||
