...
const  ResignationTable= ({resignation,setRecDelete,setFormVisible,
  setToggle,
  toggle,}) => {
...
`;
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - resignation.length) : 0;
  ...
  const convertToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(resignation);
    ...
    XLSX.writeFile(wb, "resignation.xlsx");
  };
  ...
  </Button>
  {
    ...
    <tbody className="text-center">
      {resignation.length === 0 ? renderResignationData() : (rowsPerPage > 0
            ? resignation.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : resignation
          )
          .filter((elem)=>{
            if(search.length===0)
              ...
              elem.noticeDate.toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.resignationDate.toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.resignationReason.toString().toLowerCase().includes(search.toLocaleLowerCase())
              )
