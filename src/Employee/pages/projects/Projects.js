...
const Projects = () => {
  ...
  const getProject = async () => {
    try {
      const result = await axios.get("http://13.126.190.50:5000/projects/121", {
      ...
      if (Array.isArray(result.data)) {
        const projectData = result.data[id - 1];
  ...
  e.preventDefault()
    ...
    await axios.post(
      `http://13.126.190.50:5000/projects/create/projects`,
      formData
    );
  ...
  const handleOpen = () => {
    if (formData.employeeProjectManagementEntities[0].employeeName.length > 0 && formData.employeeProjectManagementEntities.length === 1) {
      setCount(1);
                      ...
                      >
                        <TableContainer component={Paper}>
                          ...
                          >
                            ...
                            <br />
                            <TableBody>
                              {formData.employeeProjectManagementEntities.map((row, index) => (
                                <StyledTableRow
                                  key={index}
